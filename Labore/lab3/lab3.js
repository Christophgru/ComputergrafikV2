...
const moon = {
    ...
    draw: function() {
        ...
        ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI);
        let difMoonInner;
        if(this.x>canvas.width/2) difMoonInner=this.x-canvas.width/2;
        else difMoonInner=canvas.width/2-this.x;
        let difMoonOuter;
        if(this.y>canvas.height/2) difMoonOuter=this.y-canvas.height/2;
        else difMoonOuter=canvas.height/2-this.y;

        this.gradient = ctx.createLinearGradient((canvas.width/2+9*this.x)/10, (canvas.height/2+9*this.y)/10, this.x*1.1, this.y*1.1);
        this.gradient.addColorStop(0.44, 'darkgrey');
        this.gradient.addColorStop(0.52, 'white');
        ctx.fillStyle = this.gradient;
        ctx.fill();
    }
}
